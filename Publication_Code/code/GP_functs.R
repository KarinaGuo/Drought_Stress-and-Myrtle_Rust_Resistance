...
run_genomic_prediction <- function (gt_datafile, input_phenodatafile, remove_SNPs=FALSE, SNPS_TBR=NULL, trans_COI=TRUE, rm_clim_snp=FALSE, out_dir=getwd(), homo_site_filt=0.8, miss_site_filt=0.9, homo_samp_filt=0.9, miss_samp_filt=0.9, log_file){
  require(logger)
  log_appender(appender_file(file = log_file))
  ...
  if (tolower(trimws(proceed)) == "y") {
    ...
    write.csv(prediction_gt, file = paste0(wd,"prediction_gt.csv"))
  } else {
